...
TicTacToe::TicTacToe(const string& Player1, const string& Player2) : player1(Player1), player2(Player2) {}

int TicTacToe::InputBoardSize() const
    {
        int size = 0;
...
void TicTacToe::PrintBoard(const vector<vector<char>>& vec) const
    {
...
bool TicTacToe::IsValidMove(int row, int col, const vector<vector<char>>& vec) const
    {
        int size = vec.size();
        return row >= 0 && row < size && col >= 0 && col < size && vec[row][col] == '-';
    }

bool TicTacToe::IsWin(int row, int col, char currChar, const vector<vector<char>>& vec) const
    {
        int size = vec.size();
        bool rowWin = true;
        bool colWin = true;
...
bool TicTacToe::IsBoardFull(const vector<vector<char>>& vec) const
    {
